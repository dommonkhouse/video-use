...
17. **Verify the actual encoded final frame for every talking-head output.** Extract the last encoded frame and inspect a filmstrip of the final second. The speaker must not finish mid-blink, with eyes closed, or on an awkward transitional expression. If it fails, move or extend the tail to a natural eyes-open frame, re-render, and verify the replacement's actual final frame before delivery. (2026-07-16 and 2026-07-26: two Shorts reached review ending with Dom's eyes closed.)
18. **Portrait social clips need composition proof, not just 9:16 metadata.** For Shorts, Reels, and LinkedIn clips made from landscape talking-head footage, extract and inspect encoded frames before review. The speaker must read as an intentional portrait crop, not a small landscape strip over blurred filler, and titles, captions, and logos must not collide. Start on the first clean answer beat when the opening is filler-heavy, and keep captions clear of filler words unless the filler is the point. If spoken filler removal needs jump cuts, render that as a separate word-boundary proof pass. (2026-07-27: a Remotion proof was technically portrait but visually still a bad landscape stack, and the opening captions carried "um" filler.)
...
This skill owns the edit. The installed `hyperframes` router skill declares itself the mandatory entry point for any video request and HyperFrames the default output framework — that claim does not apply to a `video-use` session. Talking-head cuts, Shorts, and anything governed by the Hard Rules above stay here; call HyperFrames into an animation slot, not the other way round.
...
- **Remotion** — React/CSS compositions with component state, reusable React primitives, or an existing Remotion brand system. Best when the user specifically asks for React/Remotion or when React composition is the simpler authoring model. When Remotion is selected, load and follow the `remotion-best-practices` skill before writing or changing any Remotion code.
...
For HyperFrames slots, read the installed `hyperframes-core` skill before writing any composition HTML — it holds the authoring contract (`data-*` timing attributes, `class="clip"`, tracks, deterministic-render rules) that this section only summarises. `hyperframes-animation` covers motion, `hyperframes-cli` covers render/lint failures. Then scaffold the slot inside `edit/animations/slot_<id>/` with `npx --yes hyperframes init . --example blank --non-interactive --skip-skills`, build the HTML composition there, run the HyperFrames checks that fit the slot (`lint`, `validate`, and a draft render when practical), then produce the final overlay video with `npx --yes hyperframes render . -o render.mp4` or `--format webm -o render.webm` when alpha is required. Point the EDL overlay `file` at the actual rendered path.

Keep `--skip-skills`. The HyperFrames skills are already installed in the canonical skills repo; letting the scaffolder install them again writes into `~/.claude/skills`, which is a symlink to `~/.codex/skills` here, and creates circular symlinks that fail to resolve (2026-07-24).
